...
createButton.onclick=async()=>{audio.unlock();const name=getName();if(!name)return;setBusy(true,"Creating roomâ€¦");try{const id=await network.createGame(name);enterGame();showBanner(`ROOM ${id} â€¢ SEND THIS CODE TO PLAYER 2`,3600);}catch(e){showError(e)}finally{setBusy(false)}};
joinButton.onclick=async()=>{audio.unlock();const name=getName();if(!name)return;const code=codeInput.value.trim().toUpperCase();if(code.length!==4){status.textContent="Enter the four-character room code.";status.classList.add("error");return;}setBusy(true,"Joining roomâ€¦");try{await network.joinGame(code,name);enterGame();}catch(e){showError(e)}finally{setBusy(false)}};
startLocal.onclick=async()=>{audio.unlock();const n1=localName1.value.trim().slice(0,18),n2=localName2.value.trim().slice(0,18);if(!n1||!n2){status.textContent="Enter both player names.";status.classList.add("error");return;}localStorage.setItem("snakeBlitzLocal1",n1);localStorage.setItem("snakeBlitzLocal2",n2);setBusy(true,"Starting local co-opâ€¦");try{await network.createLocalGame(n1,n2);enterGame();}catch(e){showError(e)}finally{setBusy(false)}};
...
network.addEventListener("wave_complete",e=>{const d=(e as CustomEvent<{wave:number;repair:number;multiplier:number}>).detail;showBanner(`WAVE ${d.wave} CLEARED â€¢ ${d.multiplier.toFixed(2)}Ã— CLEAR MULTIPLIER â€¢ ROLES SWAPPED${d.repair?` â€¢ +${d.repair} HP`:""}`,3000);});
network.addEventListener("upgrade_purchased",e=>{const d=(e as CustomEvent<any>).detail;showShopMessage(`${d.purchaser} upgraded ${d.name} to Lv.${d.level} â€¢ -$${d.cost.toLocaleString()}`);audio.purchase();});
network.addEventListener("repair_purchased",e=>{const d=(e as CustomEvent<any>).detail;showShopMessage(`${d.purchaser} repaired +${d.restored} integrity â€¢ -$${d.cost.toLocaleString()}`);audio.repair();});
network.addEventListener("purchase_denied",e=>showShopMessage((e as CustomEvent<any>).detail.reason,true));
network.addEventListener("boss_phase",e=>{const d=(e as CustomEvent<any>).detail;if(d.phase==="TELEGRAPH")showBanner("BOSS TELEGRAPH â€” GET OUT OF THE STRIKE LINE!",1200);else if(d.phase==="EXPOSED")showBanner("BOSS EXPOSED â€” ACCURATE FIRE NOW!",1500);else if(d.phase==="VENOM")showBanner("VENOM BARRAGE â€” KEEP MOVING!",1200);else if(d.phase==="REINFORCEMENT")showBanner(`BOSS REINFORCEMENT INCOMING â€¢ ${d.remaining} REMAIN`,2200);});
network.addEventListener("boss_defeated",e=>{const d=(e as CustomEvent<any>).detail;showBanner(`${prettyBoss(d.type)} DEFEATED â€¢ BOSS REWARD UNLOCKED`,3200);});
network.addEventListener("boss_reward",e=>{const d=(e as CustomEvent<{reward:BoostType;name:string;description:string}>).detail;const order:BoostType[]=["SPEED","MEDKIT","REVIVE","BOMB","NUKE","CASH_BONUS"];const idx=order.indexOf(d.reward);wheel.style.setProperty("--spin-angle",`${1440+(360-idx*60)+Math.round((Math.random()-.5)*24)}deg`);bossRewardResult.textContent="SPINNINGâ€¦";bossRewardDesc.textContent="Reward locked by the server. Stand byâ€¦";spinBoss.classList.add("hidden");window.setTimeout(()=>{wheel.classList.remove("spinning");bossRewardResult.textContent=`${d.name.toUpperCase()} ACQUIRED`;bossRewardDesc.textContent=d.description;continueBoss.classList.remove("hidden");},2200);});
...
async function loadLeaderboard(target:HTMLElement){
  target.innerHTML='<div class="leaderboard-loading">Loading leaderboardâ€¦</div>';
...
function renderLeaderboard(target:HTMLElement,entries:LeaderboardEntry[],highlightRank:number|null){
  ...
  entries.slice(0,10).forEach((entry,index)=>{
    ...
    const team=document.createElement("div");team.className="leaderboard-team";const names=document.createElement("strong");names.textContent=`${entry.players[0]} + ${entry.players[1]}`;const meta=document.createElement("span");meta.textContent=`${entry.mode==="local"?"LOCAL CO-OP":"ONLINE"} â€¢ ${new Date(entry.achievedAt).toLocaleDateString()}`;team.append(names,meta);
...
function syncAudioButtons(){
 ...
 for(const b of musicButtons){b.textContent=`MUSIC â€¢ ${musicOn?"ON":"OFF"}`;b.classList.toggle("disabled-audio",!musicOn);b.setAttribute("aria-pressed",String(musicOn));}
 for(const b of sfxButtons){b.textContent=`SFX â€¢ ${sfxOn?"ON":"OFF"}`;b.classList.toggle("disabled-audio",!sfxOn);b.setAttribute("aria-pressed",String(sfxOn));}
}
...
function updateHud(s:GameSnapshot){hudRoom.textContent=s.mode==="local"?"LOCAL":s.roomId;hudWave.textContent=s.wave?`WAVE ${s.wave}`:"WAITING";hudWaveType.textContent=prettyWaveType(s.waveType);hudWaveType.dataset.type=s.waveType;hudEnemies.textContent=s.phase==="combat"?(s.boss?`${Math.ceil(s.boss.hp)} BOSS HP`:`${s.snakesRemaining} LEFT`):phaseLabel(s);hudTimer.textContent=s.phase==="intermission"?formatTime(s.timeLeftMs):s.phase==="combat"?`${Math.floor(s.waveElapsedMs/1000)}s`:"";hudScore.textContent=s.score.toLocaleString();hudCash.textContent=`$${s.cash.toLocaleString()}`;hudMultiplier.textContent=`${s.economyMultiplier.toFixed(2)}Ã—`;hudMultFill.style.width=`${Math.min(100,s.economyMultiplier*10)}%`;hudMultiplier.classList.toggle("taxed",s.economyMultiplier<1);
 ...
 const me=s.mode==="local"?undefined:s.players.find(p=>p.sessionId===network.sessionId);hudRole.textContent=s.mode==="local"?s.players.map(p=>`${p.name}: ${p.role.toUpperCase()}`).join(" â€¢ "):me?.role.toUpperCase()??"WAITING";
 waitingBanner.classList.toggle("hidden",s.players.length>=2);if(s.players.length<2)waitingBanner.textContent=`Waiting for player 2â€¦ Room ${s.roomId}`;
 if(s.mode==="local")controlsText.textContent=`${s.players.find(p=>p.role==="driver")?.name} DRIVES â€¢ ${s.players.find(p=>p.role==="gunner")?.name} GUNS â€¢ right-click cycles boosts â€¢ middle-click uses`;
 else controlsText.textContent=me?.role==="driver"?"DRIVER â€” WASD / ARROWS â€¢ kite enemies â€¢ grab cash":"GUNNER â€” MOUSE AIM â€¢ LEFT CLICK / SPACE FIRE â€¢ RIGHT CLICK CYCLE BOOST â€¢ MIDDLE CLICK USE";
 updateBoostHud(s);
}
function updateBoostHud(s:GameSnapshot){const available=s.boosts.filter(b=>b.count>0);if(!available.length){boostName.textContent="NONE";boostCount.textContent="";boostHud.classList.add("empty");return;}boostHud.classList.remove("empty");const item=available[s.selectedBoostIndex%available.length];boostName.textContent=item.name.toUpperCase();boostCount.textContent=`Ã—${item.count}`;}
function updateShop(s:GameSnapshot){const open=s.phase==="intermission";shopOverlay.classList.toggle("hidden",!open);if(!open)return;if(upgradeCards.size===0)buildUpgradeCards(s.upgrades);shopCash.textContent=`$${s.cash.toLocaleString()}`;shopTimer.textContent=formatTime(s.timeLeftMs);shopClearMultiplier.textContent=`${s.lastClearMultiplier.toFixed(2)}Ã—`;shopClearMultiplier.classList.toggle("taxed",s.lastClearMultiplier<1);shopSubtitle.textContent=`Wave ${s.wave} cleared at ${s.lastClearMultiplier.toFixed(2)}Ã—. Build for Wave ${s.wave+1}. Major upgrades visibly change the vehicle.`;shopRoles.textContent=s.players.map(p=>`${p.name} â†’ ${p.role.toUpperCase()}`).join(" â€¢ ");const st=s.combatStats;shopBuildSummary.textContent=`${st.bodyDamage} body â€¢ ${st.headDamage} head â€¢ ${(1000/st.fireIntervalMs).toFixed(1)} rps â€¢ ${st.weaponTier.replace("_"," ")} â€¢ ${st.maxHealth} HP`;
 for(const u of s.upgrades){const el=upgradeCards.get(u.id);if(!el)continue;el.level.textContent=`LV ${u.level} / ${u.maxLevel}`;el.current.textContent=u.currentEffect;el.next.textContent=u.maxed?"Maximum upgrade reached":`NEXT â€¢ ${u.nextEffect}`;el.fill.style.width=`${u.level/u.maxLevel*100}%`;el.root.classList.toggle("maxed",u.maxed);const afford=u.cost!==null&&s.cash>=u.cost;el.button.disabled=u.maxed||!afford;el.button.textContent=u.maxed?"MAXED":`$${u.cost?.toLocaleString()}`;el.button.classList.toggle("affordable",afford&&!u.maxed);}
 repairDescription.textContent=`Restore up to ${s.repair.restore} integrity â€¢ ${Math.ceil(s.tank.health)} / ${s.tank.maxHealth} HP`;buyRepairButton.textContent=s.tank.health>=s.tank.maxHealth-.5?"FULL INTEGRITY":`REPAIR â€¢ $${s.repair.cost.toLocaleString()}`;buyRepairButton.disabled=!s.repair.canBuy;
 const ready=new Set(s.readySessionIds);updateReadyPill(readyPlayer1,s.players[0],ready);updateReadyPill(readyPlayer2,s.players[1],ready);const meReady=s.mode==="local"?ready.size>=2:ready.has(network.sessionId);shopReadyButton.textContent=meReady?"READY âœ“ â€” CLICK TO CANCEL":`READY FOR WAVE ${s.wave+1}`;shopReadyButton.classList.toggle("is-ready",meReady);
}
...
function updateReadyPill(el:HTMLElement,p:GameSnapshot["players"][number]|undefined,r:Set<string>){if(!p){el.textContent="WAITING";return;}const ready=r.has(p.sessionId);el.textContent=`${p.name.toUpperCase()} â€¢ ${ready?"READY âœ“":"CHOOSING"}`;el.classList.toggle("ready",ready);}
...
function waveBanner(w:number,t:WaveType){if(t==="BOSS")return`WAVE ${w} â€¢ BOSS FIGHT â€” WATCH THE TELEGRAPH, ATTACK THE EXPOSED WINDOW`;if(t==="BONUS_MONEY")return`WAVE ${w} â€¢ CASH SURGE â€” CRATES SPAWN ~3Ã— FASTER`;if(t==="VOLATILE_SNAKES")return`WAVE ${w} â€¢ VOLATILE â€” CHAIN REACTIONS, KEEP DISTANCE`;if(t==="BLACKOUT")return`WAVE ${w} â€¢ BLACKOUT â€” LIMITED VISIBILITY`;if(t==="FRENZY")return`WAVE ${w} â€¢ FRENZY â€” FAST, DENSE HORDE`;if(t==="TITAN_NEST")return`WAVE ${w} â€¢ TITAN NEST â€” HEAVY TARGETS`;return`WAVE ${w} â€” BLITZ MULTIPLIER STARTS AT 10Ã—`;}
