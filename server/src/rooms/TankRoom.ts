...
export class TankRoom extends Room {
  ...
  private openIntermission(fromWave=false){if(fromWave){const before=this.tank.health;this.tank.health=Math.min(this.tank.maxHealth,this.tank.health+this.wave);this.swapRoles();this.broadcast("wave_complete",{wave:this.wave,score:this.score,cash:this.cash,repair:Math.round(this.tank.health-before),shopSeconds:SHOP_SECONDS,multiplier:this.lastClearMultiplier});}this.phase="intermission";this.phaseTimeLeftMs=SHOP_SECONDS*1000;this.bullets=[];this.enemyProjectiles=[];this.cashCrates.clear();this.readyPlayers.clear();this.pendingBossReward=undefined;this.resetInputs();}
  ...
  private repairSnapshot(){const cost=Math.round((165+this.wave*22)/5)*5;return{cost,restore:Math.max(1,Math.round(this.tank.maxHealth*.35)),canBuy:this.phase==="intermission"&&this.cash>=cost&&this.tank.health<this.tank.maxHealth-.5};}
